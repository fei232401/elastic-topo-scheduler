module github.com/fei/elastic-topo-scheduler
...
// out-of-tree 插件标准坑(k8s.io/kubernetes 特殊模块):
// kubernetes 仓库内部用假版本 v0.0.0 引用全部 k8s.io/* 子模块,外部项目必须全量
// replace 到真实发布版 v0.35.0(K8s 1.35 release-train 统一 tag)。
// 版本理由(设计决策 D5,见 docs/01_状态快照/Phase0_完成_三闸全绿.md)——
// k3d 默认 v1.31.5 的框架接口是内嵌版(签名不同),daocloud 无 k3s v1.36;
// v0.35 与已验证的 v0.36 接口签名级兼容(仅少 Placement/PodInPreBind),故钉 1.35。
// 独立版本模块(klog/v2、utils、kube-openapi、gengo/v2)走自身真实版本,不要 replace。
replace (
